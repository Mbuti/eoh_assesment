...
// Model for Cic entity
    models.Cic = Backbone.Model.extend({
        urlRoot: "http://localhost:31163/eoh_assesment/webresources/Cic/",
        idAttribute: 'cicid',
        defaults: {
            subject: "",
            cictype: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('cicid');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:31163/eoh_assesment/webresources/Cic/';
            }
    ...
    // Collection class for Cic entities
    models.CicCollection = Backbone.Collection.extend({
        model: models.Cic,
        url: "http://localhost:31163/eoh_assesment/webresources/Cic/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CicCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-cic-list-itemis template identifier for item
                        templateName: '#tpl-cic-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.cic = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.cic,
                    // tpl-cic-details is template identifier for chosen model element
                    templateName: '#tpl-cic-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Cic();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-cic-details is a template identifier for chosen model element
                templateName: '#tpl-cic-details',
