...
// Model for Entity entity
    models.Entity = Backbone.Model.extend({
        urlRoot: "http://localhost:31163/eoh_assesment/webresources/Entity/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('entityid');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:31163/eoh_assesment/webresources/Entity/';
            }
    ...
    // Collection class for Entity entities
    models.EntityCollection = Backbone.Collection.extend({
        model: models.Entity,
        url: "http://localhost:31163/eoh_assesment/webresources/Entity/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.EntityCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-entity-list-itemis template identifier for item
                        templateName: '#tpl-entity-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.entity = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.entity,
                    // tpl-entity-details is template identifier for chosen model element
                    templateName: '#tpl-entity-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Entity();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-entity-details is a template identifier for chosen model element
                templateName: '#tpl-entity-details',
